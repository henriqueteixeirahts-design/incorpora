"use client";

import { useActionState, useState } from "react";
import { DOCUMENT_VARIABLE_CATALOG } from "@/lib/document-variables";
import {
  createDocumentTemplateAction,
  updateDocumentTemplateAction,
  toggleDocumentTemplateStatusAction,
  type FormState,
} from "./actions";

const TYPE_LABELS: Record<string, string> = {
  SALES_CONTRACT: "Contrato de compra e venda / promessa",
  ASSIGNMENT: "Cessão de direitos",
  RESCISSION: "Distrato",
  AMENDMENT: "Aditivo contratual",
  OTHER: "Procuração / declaração / outros",
};

const initialState: FormState = {};

type Option = { id: string; label: string };

function VariableCatalog() {
  return (
    <div style={{ fontSize: "0.8rem", maxWidth: 280 }}>
      <p style={{ opacity: 0.7, marginBottom: "0.5rem" }}>
        Use <code>{"{{variavel}}"}</code> no texto. Blocos ({"{{fluxo.parcelas_tabela}}"},{" "}
        {"{{quadro_resumo}}"}) devem ficar sozinhos em um parágrafo — viram uma tabela/bloco formatado.
      </p>
      {DOCUMENT_VARIABLE_CATALOG.map((group) => (
        <div key={group.group} style={{ marginBottom: "0.75rem" }}>
          <strong>{group.group}</strong>
          <ul style={{ paddingLeft: "1.1rem", marginTop: "0.25rem" }}>
            {group.tokens.map((token) => (
              <li key={token.key}>
                <code>{`{{${token.key}}}`}</code> — {token.label}
              </li>
            ))}
          </ul>
        </div>
      ))}
    </div>
  );
}

export function NewDocumentTemplateForm({ developments }: { developments: Option[] }) {
  const [state, formAction, pending] = useActionState(createDocumentTemplateAction, initialState);

  return (
    <div className="field-section" style={{ marginTop: "1.5rem" }}>
      <h3>Novo modelo</h3>
      <div style={{ display: "flex", gap: "1.5rem", flexWrap: "wrap" }}>
        <form action={formAction} style={{ display: "flex", flexDirection: "column", gap: "0.6rem", maxWidth: 480 }}>
          <label htmlFor="new-doc-name">Nome</label>
          <input id="new-doc-name" name="name" required />

          <label htmlFor="new-doc-type">Tipo</label>
          <select id="new-doc-type" name="type" required defaultValue="">
            <option value="" disabled>
              Selecione...
            </option>
            {Object.entries(TYPE_LABELS).map(([value, label]) => (
              <option key={value} value={value}>
                {label}
              </option>
            ))}
          </select>

          <label>Empreendimentos aplicáveis (nenhum selecionado = todos)</label>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem", maxHeight: 140, overflowY: "auto" }}>
            {developments.map((dev) => (
              <label key={dev.id} style={{ fontWeight: 400 }}>
                <input type="checkbox" name="developmentIds" value={dev.id} /> {dev.label}
              </label>
            ))}
          </div>

          <label htmlFor="new-doc-content">Conteúdo</label>
          <textarea id="new-doc-content" name="content" rows={12} required />

          {state.error ? <p className="error-text">{state.error}</p> : null}

          <button type="submit" disabled={pending}>
            {pending ? "Salvando..." : "Criar modelo"}
          </button>
        </form>

        <VariableCatalog />
      </div>
    </div>
  );
}

export function EditDocumentTemplateForm({
  templateGroupId,
  type,
  currentName,
  currentContent,
  currentDevelopmentIds,
  developments,
}: {
  templateGroupId: string;
  type: string;
  currentName: string;
  currentContent: string;
  currentDevelopmentIds: string[];
  developments: Option[];
}) {
  const [open, setOpen] = useState(false);
  const [state, formAction, pending] = useActionState(updateDocumentTemplateAction, initialState);

  if (!open) {
    return (
      <button type="button" className="secondary" onClick={() => setOpen(true)}>
        Editar (cria nova versão)
      </button>
    );
  }

  return (
    <div style={{ marginTop: "0.75rem", display: "flex", gap: "1.5rem", flexWrap: "wrap" }}>
      <form action={formAction} style={{ display: "flex", flexDirection: "column", gap: "0.6rem", maxWidth: 480 }}>
        <input type="hidden" name="templateGroupId" value={templateGroupId} />
        <input type="hidden" name="type" value={type} />

        <label htmlFor={`edit-doc-name-${templateGroupId}`}>Nome</label>
        <input id={`edit-doc-name-${templateGroupId}`} name="name" defaultValue={currentName} required />

        <label>Empreendimentos aplicáveis (nenhum selecionado = todos)</label>
        <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem", maxHeight: 140, overflowY: "auto" }}>
          {developments.map((dev) => (
            <label key={dev.id} style={{ fontWeight: 400 }}>
              <input
                type="checkbox"
                name="developmentIds"
                value={dev.id}
                defaultChecked={currentDevelopmentIds.includes(dev.id)}
              />{" "}
              {dev.label}
            </label>
          ))}
        </div>

        <label htmlFor={`edit-doc-content-${templateGroupId}`}>Conteúdo</label>
        <textarea
          id={`edit-doc-content-${templateGroupId}`}
          name="content"
          rows={12}
          defaultValue={currentContent}
          required
        />

        {state.error ? <p className="error-text">{state.error}</p> : null}
        {state.ok ? (
          <p style={{ fontSize: "0.85rem", opacity: 0.8 }}>Nova versão salva.</p>
        ) : null}

        <div style={{ display: "flex", gap: "0.5rem" }}>
          <button type="submit" disabled={pending}>
            {pending ? "Salvando..." : "Salvar como nova versão"}
          </button>
          <button type="button" className="secondary" onClick={() => setOpen(false)}>
            Fechar
          </button>
        </div>
      </form>

      <VariableCatalog />
    </div>
  );
}

export function ToggleDocumentTemplateStatusButton({
  templateGroupId,
  status,
}: {
  templateGroupId: string;
  status: "ACTIVE" | "INACTIVE";
}) {
  const nextStatus = status === "ACTIVE" ? "INACTIVE" : "ACTIVE";
  return (
    <form action={toggleDocumentTemplateStatusAction}>
      <input type="hidden" name="templateGroupId" value={templateGroupId} />
      <input type="hidden" name="status" value={nextStatus} />
      <button type="submit" className="secondary">
        {status === "ACTIVE" ? "Inativar" : "Ativar"}
      </button>
    </form>
  );
}
